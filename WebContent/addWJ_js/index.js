function a() {
    var fd = new FormData();
    var selector = $(".wjdc_list");
    var len = selector.length;
    for(var i=0;i<len;i++){
        fd.append("t"+(i+1).toString(), ($(selector.get(i)).find(".btwenzi").html()).toString());
        var o = $(selector.get(i)).find(".option");
        for(var j=0;j<o.length;j++){
            fd.append("o"+(i+1)+(j+1).toString(),($(o.get(j)).html()).toString());
		}
		fd.append("l"+(i+1).toString(),o.length);
    }
    fd.append("len", len);
    fd.append('w_type', $($("select[name='w_type']").get(0)).val());
    fd.append('w_title', $($("textarea[name='w_title']").get(0)).val());
    fd.append('endtime', $($("input[name='endtime']").get(0)).val());
    ...
    $.ajax({
        ...
        success: function (response) {
            window.location.href="/OnlineSurveySystem/view_WJ.jsp"
        }
    });
}
			 ...
			 $('#addquerstions').change(function() {
		            ...
		            switch (index) {
                        ...
                        case "2": //问答
		                    ...
		                    wjdc_list = $(wjdc_list).append(' <li><div class="tm_btitlt"><i class="nmb">' + Grade + '</i>. <i class="btwenzi" name="title">请编辑问题？</i><span class="tip_wz">' + danxuan + '</span></div></li>');
		                    if (index == "2") {
					...
					switch(querstionType) {
						...
						case "1": //多选	
							...
							jcxxxx.children(".title_itram").children(".kzjxx_iteam").each(function() {
								...
								var li = '<li><label><input name="a" type="' + inputType + '" value=""><span class="option">' + texte_val_bj + '</span></label></li>';
